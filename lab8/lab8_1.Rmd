...
```{r}
relig_income <- readr::read_csv(here("lab8", "data", "relig_income.csv"))
relig_income
```
...
```{r}
relig_income %>% 
  pivot_longer(-religion, names_to="incom", values_to="count")
```
...
```{r}
plant_data %>% 
  pivot_longer(cols=starts_with("day"), names_to="day", names_prefix="day", values_to="measurement") %>% 
